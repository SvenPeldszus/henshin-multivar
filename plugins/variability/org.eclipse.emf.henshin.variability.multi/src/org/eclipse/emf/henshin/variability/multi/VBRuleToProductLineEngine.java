...
public class VBRuleToProductLineEngine extends VariabilityAwareMatcher {
	...
	public VBRuleToProductLineEngine(Rule rule, EGraph graphP, IFeatureModel fmP, Map<EObject, String> pcsP,
			RuleToProductLineEngine varEngine) throws InconsistentRuleException {
		super(rule, graphP);
		this.fmP = fmP;
		this.pcsP = pcsP;
	...
	public void prepareRulePreparators(List<RulePreparator> preparators, List<Set<Sentence>> assumedTrue)
		...
		for (int i = 0; i < trueFeatureList.size(); i++) {
			...
			Set<Sentence> elementsToRemove = RuleUtil.calculateElementsToRemove(ruleInfo, trueFeatures, falseFeatures);
