...
//		Module module = rs.getModule("rules\\generated\\CREATE\\rr_CREATE_Class_IN_Class_(nestedClassifier)_execute.henshin");
		...
		for (int choice = 0; choice < values.length; choice++)
			for (RuleSet set : RuleSet.values()) {
				...
				RuntimeBenchmarkReport reporter = new RuntimeBenchmarkReport(projectPath, UmlRecogBenchmarkClassic.class.getSimpleName(),
				...
				for (String example : examples) {
					...
					try {
						new UmlRecogBenchmarkClassic().runPerformanceBenchmark(module, set, example, projectPath, reporter);
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
	 ...
	 */
	@Override
	public void runPerformanceBenchmark(Module module, RuleSet set, String exampleID, File project,
